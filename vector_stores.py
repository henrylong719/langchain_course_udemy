...
def chroma_basics():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        query = "What is LaChin"
        results = vectorstore.similarity_search(query, k=2)

        print(f"Top 2 resuts for query '{query}':")
...
def exercise_vector_store_setup():
    ...
    def create_retriever(
        text: list[str], chunk_size: int = 500, chunk_overlap: int = 50, k: int = 3
    ):
    ...
    queries = ["What's good for web development?", "What language is the safest"]
