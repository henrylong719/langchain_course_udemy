...
def demo_basic_chain():
    ...
    result = chain.invoke(
        {"text": "LangChain is a framework for developing applications powered by llm"}
    )
...
def demo_parallel_chain():
    ...
    summarize_prompt = ChatPromptTemplate.from_template(
        "Summarize in two sentence: {text}"
    )

    keywords_prompt = ChatPromptTemplate.from_template(
        "Extract 5 keywords in the following text: {text}\nReturn as a comma-separate sentence"
    )
...
def demo_debugging():
    ...
    result = chain.with_config(run_name="greeting_chain").invoke({"name": "Henry"})
