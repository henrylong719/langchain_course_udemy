...
def create_map_reduce_summarizer():
    ...
    async def map_summarize(state: MapReduceState) -> dict:
        ...
        summaries = await asyncio.gather(
            *(summarize_one(doc) for doc in state["documents"])
        )
        return {"summaries": summaries}

    # def map_summarize(state: MapReduceState) -> dict:
    #     """Summarize each document (not runs in parallel for each)."""
    #     summaries = []
